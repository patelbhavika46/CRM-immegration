...
function LogActivityModal({ applicantId, onClose }: { applicantId: number; onClose: () => void }) {
...
export default function CommunicationsTab({ applicantId }: Props) {
  ...
  return (
    <div>
      ...
      {modalOpen && <LogActivityModal applicantId={applicantId} onClose={() => setModalOpen(false)} />}
    </div>
  );
}
