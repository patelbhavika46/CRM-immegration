...
function NewTaskModal({ clientId, onClose }: { clientId: number; onClose: () => void }) {
...
function TaskRow({ task, clientId }: { task: Activity; clientId: number }) {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ['clients', clientId, 'activities'] });
    } finally {
      setCompleting(false);
    }
  };
...
export default function TasksTab({ clientId }: Props) {
  ...
  const { data, isLoading } = useClientActivities(clientId, { page, per_page: 20, type: TASK_TYPES });
  const pending   = data?.data.filter(t => t.status !== 'completed' && t.status !== 'cancelled') ?? [];
  const completed = data?.data.filter(t => t.status === 'completed' || t.status === 'cancelled') ?? [];
        ...
        <div className="space-y-4">
          {pending.length > 0 && (
            <div>
              ...
              <div className="space-y-2">{pending.map(t => <TaskRow key={t.id} task={t} clientId={clientId} />)}</div>
            </div>
          )}
          {completed.length > 0 && (
            <div>
              <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Completed</h4>
              <div className="space-y-2">{completed.map(t => <TaskRow key={t.id} task={t} clientId={clientId} />)}</div>
            </div>
          )}
        </div>
      )}

      {newTaskOpen && <NewTaskModal clientId={clientId} onClose={() => setNewTaskOpen(false)} />}
    </div>
  );
}
