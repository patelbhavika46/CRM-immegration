...
import { useClientDocuments, useUploadDocument, useDeleteDocument } from '../../../hooks/useClients';
...
function UploadModal({ clientId, onClose }: { clientId: number; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    fd.append('file',              file);
    fd.append('documentable_type', 'client');
    fd.append('documentable_id',   String(clientId));
...
function DocumentRow({ doc, clientId, onDelete }: { doc: ClientDocument; clientId: number; onDelete: (id: number) => void }) {
  ...
  const handleDownload = async () => {
    setDownloading(true);
    try {
      const response = await clientsApi.downloadDocument(doc.id);
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {docs.map(doc => (
            <DocumentRow key={doc.id} doc={doc} clientId={clientId} onDelete={id => deleteDoc.mutate(id)} />
          ))}
        </div>
      )}

      {uploadOpen && <UploadModal clientId={clientId} onClose={() => setUploadOpen(false)} />}
    </div>
  );
}
